<template>
	<view class="page">
		...
		<scroll-view id="tab-bar" class="scroll-h" :scroll-x="true" :show-scrollbar="false"
			...
			<view v-for="(tab,index) in list" :key="index" class="uni-tab-item" :id="index" :data-current="index"
				@click="ontabtap">
				<text class="uni-tab-item-title"
					:class="tabIndex==index ? 'uni-tab-item-title-active' : ''">{{tab.BillNo}}</text>
			</view>
		</scroll-view>
		<view class="card-box" v-for="(info,index1) in list" :key="index1" v-show="tabIndex == index1 ">
			...
			<uni-card>
				<uni-list>
					...
					<uni-list-item title="电话号码" :rightText="info.FSENDPHONE" />
					<uni-list-item title="发货时间" :rightText="resetTime(info.FREPAIRRPTIME)" />
					<uni-list-item title="关联工单编号" :rightText="info.FWORDERNO" />
					<uni-list-item title="物流单号" :rightText="info.FWLDH" clickable
						@click="copyToClipboard(info.FWLDH)" />
					<uni-list-item title="发运方式" :rightText="info.FWLGS" />
					<!-- <uni-list-item title="运费" :rightText="info.FFREIGHT"  产品/配件名  产品编号/> -->
					...
					<uni-list-item title=" " v-for="(item,index) in info.FEntity" :key='item.Id'>
						<template v-slot:header>
							<text
								style="font-size: 12px;color: black;display: flex;width: 25%;">{{item.FPARTSNAME.Name[0].Value}}</text>
						</template>

						<template v-slot:body>
							<!-- FORDERNO  Specification-->
							<view style="font-size: 12px;color: black;width: 65%;">
								<span
									style="width: 50%;text-align: center;float: left;">{{item.FPARTSNAME.Specification[0].Value}}</span>
								<span style="width: 50%;text-align: center;float: left;">{{item.FNEWNUMBER}}</span>
								<!-- <span style="width: 50%;text-align: center;float: right;" >{{item.FPRODUCTNO}}</span> -->
							</view>
						</template>
					...
					<!-- <uni-list-item title=" " v-for="(item,index) in info.FEntity" :key='item.Id'>
						<template v-slot:header>
							<text style="font-size: 12px;color: black;display: flex;width: 55%;">{{item.FPARTSNAME.Name[0].Value}} 【{{item.FORDERNO}}】</text>
						</template> 
						<template v-slot:body> 
					...
					<view style="font-size: 12px;color: black;width: 90%;text-align: left;padding: 12px 15px;">
						{{info.FHEADREMARKS}}
					</view>
					<!-- <textarea   style="padding: 10px;border: 1px solid #c3c3c3;height: 80px;"  disabled="true" v-model="info.FHEADREMARKS" cols="20" rows="3" placeholder="请输入备注信息"></textarea> -->
				</uni-list>
			</uni-card>
			<!-- 收货信息 FRECEIVERTEL -->
			<uni-card>
			...
			<!-- 保存确认 -->
			<u-modal :show="showModel" :closeOnClickOverlay="true" :showCancelButton="true"
				@confirm="confirmReceive(info.BillNo)" :title="'确认收货'" @cancel="showModel=false"
				@close="showModel=false"></u-modal>
		</view>
