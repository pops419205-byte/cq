	...
	<view class="bg-image-box">    
		...
		<uni-card>
			<uni-list>
				...
				<uni-list-item title="物流单号" :rightText="info.FWLDH " clickable @click="copyToClipboard"	/>  
				...
				<view style="font-size: 12px;color: black;width: 90%;text-align: left;padding: 12px 15px;">
					 {{(info.FHEADREMARKS==' '|| info.FHEADREMARKS=='')?'无':info.FHEADREMARKS}}
				</view>
			</uni-list>
		...
		<uni-card>
			<uni-list>
				...
				<uni-list-item title="电话号码" :rightText="(info.FSENDPHONE!==' ' && info.FSENDPHONE!=='')?info.FSENDPHONE:info.FSENDER.TEL" /> 
				...
				<uni-list-item title=" ">
					...
					<template v-slot:footer>
						<view style="width: 200px;font-size: 12px;color: black;text-align: end;">
							{{(info.FSENDADDRESS!==' ' && info.FSENDADDRESS!=='')?info.FSENDADDRESS:info.FSENDER.ADDRESS}} 
						</view>
					</template>
				</uni-list-item>
			</uni-list>
...
<script>
	...
	export default {
		...
		methods: {   
			...
			copyToClipboard(){
				console.log(11111111111,this.info.FWLDH)
