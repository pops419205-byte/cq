		...
		<view class="main">
			...
			<view class="button-box">
				...
				<button size="medium" type="warning" v-if="isAuthor" style="color: black;margin-top: 20rpx"
					...
					<!-- 调试登录 -->
				<button v-if="process.env.NODE_ENV === 'development'" 
				        @click="debugLogin" 
...
<script>
	export default {
		 ...
		 */
		onShow() {
			console.log("onshow--privacyAgree--", this.$store.state.privacyAgree)
			console.log("login--onshow--showprivacy--", this.showPrivacy)
			console.log("stor--userRoleList--", uni.getStorageSync('userRoleList'))
		...
		methods: {
			 ...
			 */
			async getMiniUser() { // 获取用户信息
				console.log('getMiniUser开始执行')
				let data = {
				...
				if (res.data.length == 0) {
					...
					if (res.data[0].customertype == 'Service_station') { // 服务站
					...
					if (this.status == 'Distributor') { // 经销商
						uni.$emit('aEven', '2')
						this.totalNav('Distributor')
					} else {  // 普通用户 ,跳转到driver页
						console.log("status,res.data[0].customertype--",res.data[0].customertype)
					...
					const result = list.filter(item => {
						if (item.customertype == 'Service_station') { // 筛选出服务商身份
							return item
						}
					})
					let insertNum = 0;
					...
					console.log('getMiniUser,insertNum--', insertNum)
					let _thisa = this;
					if (insertNum > 0) {
						setTimeout(function() {
							_thisa.getMiniUser();
					...
					let data
					if (result.length > 0) { // 服务商
						data = result[0];
					} else { // 中心库
						data = list[0]
					}
					console.log("getMiniUser,data--", data, 444)
			...
			wxGetUserInfo() { // 页面按钮“授权微信登录""
				uni.getUserProfile({
					...
					fail: (err) => {
						console.log("登录失败，wxGetUserInfo，err--", err)
						uni.login({
							provider: 'weixin',
							success: ({
								code
			...
			nav(type) {
				...
				if (type == 'Customer') {
					console.log("跳转到普通用户（司机)注册页，type--",type)
					uni.redirectTo({
