		...
		<scroll-view style="height: calc( 100vh - 32px ) ;" scroll-y="true" refresher-enabled="true"
			...
			<template>
				...
				<view class="card" v-for="(item,index) in listArr" :key='index'>
					<!-- 卡片头部-产品名称 -->
					<view class="head" @click="infoBtn(item.FBILLNO,item.FWORDERNO)">
						<view class="left">
							<text class="materiel-name">工单编号: {{item.FWORDERNO }}</text>
						</view>
					</view>
					<!-- 工单信息 -->
					<view class="middle" @click="infoBtn(item.FBILLNO,item.FWORDERNO)">
	...
	export default {
		data() {
			return { 
				...
				items: ['补货中','已签收' ], 
				listArr:[],
                Icon1: { color: '#4cd964', size: '22', type: 'calendar' } ,
                Icon2: { color: '#4cd964', size: '22', type: 'vip' } ,
                Icon3: { color: '#4cd964', size: '22', type: 'loop' } ,
                Icon4: { color: '#4cd964', size: '22', type: 'location' } ,
		...
		methods: {   
			...
			initB(){ 
				...
				let data = {
				 ...
				 _this.listArr = [] ;
				_this.$Recipe.GetAllReceiveBySender(data).then((res) => {
					console.log(res)
			...
			infoBtn(num,FWORDERNO){    
				let obj ={
					FBillNo:num,
					FWORKORDERNO1:FWORDERNO,
					tabIndex:this.tabIndex
				}
				let url = this.$Router.my.Replenishzxk +'?obj='+encodeURIComponent(JSON.stringify(obj));
