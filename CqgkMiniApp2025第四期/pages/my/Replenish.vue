		...
		<view class="card-box" v-for="(info,index1) in list" :key="index1" v-show="tabIndex == index1 ">
			...
			<uni-card>
				<uni-list>
					...
					<!-- <uni-list-item title="发货地址" :rightText="info.FSENDER.ADDRESS"   />   FSENDER.ADDRESS-->
					<uni-list-item title=" ">
					...
					<uni-list-item title=" " v-for="(item,index) in info.FEntity" :key='item.Id'>
						...
						<!-- item.FPRODUCTNO?item.FPRODUCTNO:item.FPARTSNAME.Specification[0].Value -->
						<template v-slot:body>
							<view style="font-size: 12px;color: black;width: 65%;">
								<span
									style="width: 50%;text-align: center;float: left;">{{item.FPARTSNAME.Specification[0].Value}}</span>
								<span
									style="width: 50%;text-align: center;float: right;">{{item.FNEWNUMBER?item.FNEWNUMBER:'/'}}</span>
							</view>
						</template>
			...
			<uni-card>
				<uni-list>
					...
					<uni-list-item title="收货人电话" :rightText="info.FRECEIVERTEL" />
					<uni-list-item title="收货方" :rightText="info.FRECEIVER.Name[0].Value" />
					<uni-list-item title="收货方电话" :rightText="info.FRECEIVER.TEL" />
