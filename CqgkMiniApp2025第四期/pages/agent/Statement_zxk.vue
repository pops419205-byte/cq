				...
				<uni-forms ref="baseForm" :modelValue="formData">
					<uni-forms-item label="结算时间">
						<!-- <uni-datetime-picker  placeholder="请选择结算时间" :showSale="false" type="datetime" hide-second="true" :clearIcon="false" @change="getNewList"
							...
							<picker mode="date" class="picker-style" selector-type="select" fields="year" start="2021-01-01" end="2999-12-20" :value="formData.time"  @change="bindDateChange">
	...
	export default {
		...
		methods: {   
			...
			init(){ 
				...
				return
				_this.$Recipe.CenterSettlementInfo(data).then((res) => { 
					_this.info = res.data 
					// _this.WEIFU = 0
				})
			},
			...
			orderBtn(item) { 
				uni.navigateTo({
					url: '/pages/agent/Statementinfo?obj=' + encodeURIComponent(JSON.stringify(item))
				}); 
			},
			bindDateChange(e){
				console.log(e)
