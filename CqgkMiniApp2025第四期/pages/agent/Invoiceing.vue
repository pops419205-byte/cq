<template>
	<view class="bg-image-box">
			...
			<uni-list>
				...
				<uni-forms ref="baseForm" :modelValue="formData">
					...
					<uni-forms-item label="发货时间" required>
						<uni-datetime-picker placeholder="请选择发货时间" :showSale="false" type="datetime" hide-second="true"
							:clearIcon="false" v-model="formData.FREPAIRRPTIME" />
					</uni-forms-item>
					...
					<uni-forms-item label="备注信息">
						<u--textarea v-model="formData.FHEADREMARKS" :maxlength='-1'
		...
		<!-- <button   type="warning" class="addbtn"  @click="addBtn">确认发货</button> -->
		...
		<u-picker :show="show" :closeOnClickOverlay='true' @cancel='show=false' @close='show=false'
			@confirm='TypeConfirm' :columns="typeList" keyName="name"></u-picker>
...
<script>
	...
	export default {
		data() {
			return {
				...
				formData: {
					FWLDH: '',
					FWLGS: '',
					FREPAIRRPTIME: this.happenTimeFun(),
					FFREIGHT: 0,
					FSENDERNAME: '',
					FSENDPHONE: '',
					FSENDADDRESS: '',
		...
		onLoad(options) {
			console.log('onLoad,FWORDERNO', options)
		...
		methods: {
			...
			init() {
				...
				_this.$Recipe.GetSendByFnumber(data).then((res) => {
					...
					if (res.data) {
						...
						_this.formData.FHEADREMARKS = res.data.FHEADREMARKS;
			...
			addBtn() {
				...
				let data = {
					...
					FHEADREMARKS: _this.formData.FHEADREMARKS,
				...
				// 发货信息
				console.log("发货信息，FREPAIRRPTIME--", this.formData)
				...
				// debugger
				// return
				this.$Recipe.Savefahuo(data).then((res) => {
					console.log("发货确认返还，res--", res)
					if (res?.msg == 'Success') {
						//uni.$emit('reLoadAgent', '1') 
						uni.showToast({
							icon: 'none',
							title: '发货成功'
						})
